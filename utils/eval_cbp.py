...
def eval_mpra():
    ...
    CBPpredsR = CBP.predict([XR_test], verbose=True)[:][0] #1
    CBPpredsA = CBP.predict([XA_test], verbose=True)[:][0] #1
    CBPpreds = []
    ...
    results.to_csv('metrics/cbpmetricsjsd.csv')
    np.save('metrics/cbppredsjsd', np.array(CBPpreds))
    print(results.info())
