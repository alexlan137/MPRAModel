...
def eval_mpra():
    ...
    CBP = load('/wynton/home/corces/allan/MPRAModel/Soumya/mitra.stanford.edu/kundaje/soumyak/chromatin-atlas-2022/ATAC/ENCSR095QNB/chrombpnet_model_feb15/chrombpnet_wo_bias.h5')
    print("CBP loaded from file")
    print(CBP.summary())

    XR_test = np.load('data/MPRA_partitioned/AbellV2pval/XR.001.npy')
    XA_test = np.load('data/MPRA_partitioned/AbellV2pval/XA.001.npy')
    y_test = np.load('data/MPRA_partitioned/AbellV2pval/delta.001.npy')
    ...
    results.to_csv('MPRA_data_analysis/Abell/AbellV2_CBPmetrics_pval/pval0.001_Soumya_QNB.csv')
    np.save('MPRA_data_analysis/Abell/AbellV2_CBPmetrics_pval/pval0.001_Soumya_QNB', np.array(CBPpreds))
    print(results.info())
